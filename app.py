...
def load_all():
    ...
    if not parquet_path.exists():
        try:
            ...
            with st.spinner("Converting Excel files to Parquet (first run)..."):
...
def preprocess_data(_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ensure entity
    entity_col = safe_col(df, ['entity','company','brand','entity_name', 'buying_legal_entity'])
    if entity_col and entity_col in df.columns:
        # Use column if available, but fallback to source file if missing/empty for specific rows
        df['entity'] = df[entity_col].fillna('').astype(str).str.strip()
        # If the column resulted in empty strings, try to fill from entity_source_file
        if 'entity_source_file' in df.columns:
            mask_empty = df['entity'] == ''
            df.loc[mask_empty, 'entity'] = df.loc[mask_empty, 'entity_source_file'].fillna('').astype(str).str.strip()
    else:
