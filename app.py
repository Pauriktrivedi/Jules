...
def load_all():
    ...
    if not parquet_path.exists():
        st.warning("Data file (p2p_data.parquet) not found. Attempting to generate from source Excel files...")
        try:
            from convert_to_parquet import convert_all_to_parquet
            convert_all_to_parquet()
        except Exception as e:
            st.error(f"Failed to generate parquet file: {e}")
            return pd.DataFrame()

    if not parquet_path.exists():
         st.error("Data file could not be generated.")
         return pd.DataFrame()
    ...
    try:
        ...
        st.warning(f"Failed to load existing Parquet file: {e}. Attempting regeneration...")
        try:
            from convert_to_parquet import convert_all_to_parquet
            convert_all_to_parquet()
            if parquet_path.exists():
                st.success("Regenerated p2p_data.parquet successfully. Reloading...")
                df = pd.read_parquet(parquet_path)
                for c in ['pr_date_submitted', 'po_create_date', 'po_delivery_date', 'po_approved_date']:
                    if c in df.columns:
                        df[c] = pd.to_datetime(df[c], errors='coerce')
                return df
                 ...
                 st.error("Regeneration appeared to succeed but file is missing.")
                 return pd.DataFrame()
        except Exception as e2:
            st.error(f"Failed to regenerate Parquet file: {e2}")
            return pd.DataFrame()
        ...
        if (pr_qty_col or pr_unit_rate_col or pr_value_col) and (po_qty_col or po_unit_rate_col or net_col):
            try:
                ...
                if savings_df.empty:
                    ...
                    fig_hist = px.histogram(savings_df, x='savings_pct', nbins=50, title='% Saved Distribution (Bell Curve)', 
                                            labels={'savings_pct':'% Saved'}, marginal="rug")
                    c_h1.plotly_chart(fig_hist, use_container_width=True)
    
                    # Box Plot
                    fig_box = px.box(savings_df, x='savings_pct', title='% Saved Outliers (Box Plot)', points="outliers")
                    c_h2.plotly_chart(fig_box, use_container_width=True)
